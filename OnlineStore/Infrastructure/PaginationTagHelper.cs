...
namespace OnlineStore.Infrastructure
{
    public class PaginationTagHelper : TagHelper
    {
        ...
        private void ClosingHtml(StringBuilder content)
        {
            ...
            {
                content.Append($"<li class='page-item'><a class='page-link' href='{PageTarget}?productPage={PageCount}'>{PageLast}</a></li>");
            }
            content.Append(" </ul");
        }
        ...
        private void PageNumberUpperPageRange(StringBuilder content)
        {
            for (int currentPage = PageCount - (2 * PageRange); currentPage < PageCount + 1; currentPage++)
            {
                ...
                content.Append($"<li class='page-item {active}'><a class='page-link'href='{PageTarget}?productPage={currentPage}'>{currentPage}</a></li>");
            }
        }
